...
def _require_session(shop_name: str) -> None:
    if not has_session(shop_name):
        raise SessionExpired(
            f"No saved session for '{shop_name}'. Log in to it first."
        )
...
def login(shop_name: str, headless: bool = False) -> bool:
    ...
    with browser_page(shop.name, headless=headless, use_session=False) as (page, context):
            ...
            try:
                if shop.is_logged_in(page):
                    ...
                    if confirmed >= 2:  # two consecutive checks = stable
                        context.storage_state(path=session_path(shop.name))
                        return True
                else:
                    confirmed = 0
            except Exception:
